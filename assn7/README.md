...
알고리즘이 아닌 단위 테스트의 코드일 뿐인데도 코드리뷰를 하며 조원들과 코드가 달랐다는 것에 놀랐다. 테스트 케이스가 다른 것 이외로도, 내가 생각하지 못해 추가하지 않았었던 중복된 스펠링을 쓰는 경우 등등을 코드리뷰 하며 조원들에게 배웠고, 수업이 끝난 후 내가 추가적으로 생각한 것 까지 추가해 테스트를 해 주었다. 또, 내가 이번 코드를 짤 때는 하나의 테스트 케이스를 이용해 모든 함수를 테스트 해 주었다. 하지만 수업 시간에 교수님이 하나의 함수 테스트를 할 때마다 하나의 테스트 케이스를 새로 만들어 주는 것이 바람직하다 말씀해 주셨고, 다음 과제와 ad 프로젝트에 대해 단위 테스트를 할 땐 그런 방식으로 시도해 봐야겠다는 생각이 들었다. 그리고 세세하게 테스트 케이스를 짜고 테스트 경우를 짠다면, 어느 함수에 어떤 부분에 오류가 있는지도 알아낼 수 있어 코드를 수정할 수 있기 때문에 단위 테스트가 참 중요하다는 것도 깨달았다.
